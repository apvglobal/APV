...
namespace Apv.Network
{
    public class LocalNode : IDisposable
    {
        ...
        public LocalNode()
        {
            ...
            this.UserAgent = string.Format("/Apv:{0}/", GetType().GetTypeInfo().Assembly.GetName().Version.ToString(3));
                ...
                Task.Run(async () =>
                {
                        ...
                        && await UPnP.DiscoverAsync())
                    {
                        try
                        {
                            ...
                            if (port > 0)
                                await UPnP.ForwardPortAsync(port, ProtocolType.Tcp, "Apv");
                            if (ws_port > 0)
                                await UPnP.ForwardPortAsync(ws_port, ProtocolType.Tcp, "Apv WebSocket");
                        }
                        catch { }
                    }
                    connectThread.Start();
